<template>
  <div>
    ...
    <div v-if="projetsPretsADeliberer.length" class="mb-4">
      <div class="vld-section-label">Projets prêts à délibérer</div>
      <div class="d-flex flex-column gap-2">
        <div v-for="p in projetsPretsADeliberer" :key="p.jury_id" class="card" style="border-left:4px solid var(--vld-accent)">
          <div class="card-body d-flex align-items-center gap-3 flex-wrap">
            <div class="flex-grow-1">
              <div class="fw-bold" style="color:var(--vld-text-strong)">{{ p.etudiant_nom }}</div>
              <div style="font-size:13px;color:var(--vld-text-muted)">{{ p.projet_titre||'—' }} · {{ p.nb_fiches||0 }} fiche(s) soumise(s)</div>
            </div>
      ...
      <table class="table table-hover mb-0">
        ...
        <tbody>
          <tr v-for="r in resultatsFiltered" :key="r.id">
            ...
            <td>
              <span class="badge" :class="r.decision==='admis' ? 'bg-success' : 'bg-danger'">
...
export default {
  ...
  emits: ['toast'],
  data () { return { loadingResultats:false, loadingPublier:false, resultats:[], projetsPretsADeliberer:[], searchDelib:'' } },
  computed: {
    resultatsFiltered () { if(!this.searchDelib) return this.resultats; const q=this.searchDelib.toLowerCase(); return this.resultats.filter(r=>r.etudiant_nom.toLowerCase().includes(q)||(r.projet_titre||'').toLowerCase().includes(q)) },
  },
  mounted () { this.chargerResultats() },
  methods: {
    initiales (n) { return (n||'?').split(' ').map(w=>w[0]).join('').slice(0,2).toUpperCase() },
    mentionClass (m) { return { 'Très bien':'bg-success', 'Bien':'bg-primary', 'Assez bien':'bg-info text-dark', 'Passable':'bg-warning text-dark', 'Insuffisant':'bg-danger' }[m]||'bg-secondary' },
    async chargerResultats () {
      this.loadingResultats=true
      try { const [r,p]=await Promise.all([api.get('/resultats-pfe'),api.get('/jurys-pfe/prets-a-deliberer')]); this.resultats=r.data||[]; this.projetsPretsADeliberer=(p.data||[]).map(x=>({...x,loading:false})) }
      catch { this.$emit('toast',{type:'toast-err',message:'Erreur chargement des résultats.'}) }
      finally { this.loadingResultats=false }
    },
    async deliberer (p) {
      p.loading=true
      try { const{data}=await api.post(`/jurys-pfe/${p.jury_id}/deliberer`); this.resultats.unshift({id:data.id,jury_id:p.jury_id,etudiant_nom:p.etudiant_nom,projet_titre:p.projet_titre,note_finale:data.note_finale,mention:data.mention,decision:data.decision,publie:false,en_biblio:false,archive:false}); this.projetsPretsADeliberer=this.projetsPretsADeliberer.filter(x=>x.jury_id!==p.jury_id); this.$emit('toast',{type:'toast-ok',message:`Délibération enregistrée — ${p.etudiant_nom} : ${data.note_finale}/20`}) }
      catch(e) { this.$emit('toast',{type:'toast-err',message:e.response?.data?.message||'Erreur lors de la délibération.'}) }
      finally { p.loading=false }
    },
    async publierResultat (r) { try { await api.post(`/jurys-pfe/${r.jury_id}/publier`); r.publie=true; this.$emit('toast',{type:'toast-ok',message:`Résultat publié — ${r.etudiant_nom} notifié.`}) } catch { this.$emit('toast',{type:'toast-err',message:'Erreur.'}) } },
    async publierTous () { this.loadingPublier=true; try { await api.post('/resultats-pfe/publier-tous'); this.resultats.forEach(r=>r.publie=true); this.$emit('toast',{type:'toast-ok',message:'Tous les résultats ont été publiés.'}) } catch { this.$emit('toast',{type:'toast-err',message:'Erreur.'}) } finally { this.loadingPublier=false } },
    async ajouterBiblio (r) { try { await api.post(`/resultats-pfe/${r.id}/bibliotheque`); r.en_biblio=true; this.$emit('toast',{type:'toast-ok',message:`${r.etudiant_nom} ajouté à la bibliothèque PFE.`}) } catch { this.$emit('toast',{type:'toast-err',message:'Erreur.'}) } },
    async archiverResultat (r) { try { await api.post(`/resultats-pfe/${r.id}/archiver`); r.archive=true; this.$emit('toast',{type:'toast-ok',message:`${r.etudiant_nom} archivé.`}) } catch { this.$emit('toast',{type:'toast-err',message:'Erreur.'}) } },
  },
}
</script>