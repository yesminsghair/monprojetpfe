  ...
  <div class="modal-overlay" @click.self="$emit('fermer')">
    <div class="modal-box">
      ...
      <div class="modal-body" v-if="chef">
        ...
        <form @submit.prevent="confirmer" novalidate>
          ...
          <div class="field-block">
            ...
            <input v-model="form.domaineExpertise" type="text"
      ...
      <div class="modal-footer">
        <button class="btn-blue" :disabled="saving" @click="confirmer">
          <span v-if="saving" class="vld-spinner-sm" style="width:14px;height:14px"></span>
...
export default {
  ...
  watch: {
    chef: {
      ...
      handler(val) {
        if (val) {
          // ChefController::format() returns: nom, prenom, email, telephone, domaineExpertise
          this.form = {
            nom:             val.nom,
            prenom:          val.prenom,
            email:           val.email,
            telephone:       val.telephone        || '',
            domaineExpertise:val.domaineExpertise || '',
          }
          this.errors = {}
        }
      },
  ...
  methods: {
    validate() {
      ...
      if (this.form.email && rx.test(this.form.email)) {
        const doublon = this.chefs.some(
          c => c.id !== this.chef.id &&
               c.email.toLowerCase() === this.form.email.toLowerCase()
        )
    ...
    async confirmer() {
      ...
      this.saving = true
      try {
        // PUT /api/chefs/{id}/modifier  (ChefController::modifier)
        // Validation keys: nom, prenom, email, telephone, domaineExpertise  (all camelCase)
        const r = await api.put(`/chefs/${this.chef.id}/modifier`, {
          nom:              this.form.nom,
          prenom:           this.form.prenom,
          email:            this.form.email,
          telephone:        this.form.telephone        || null,
          domaineExpertise: this.form.domaineExpertise || null,
        })
        this.$emit('modifie', r.data)
        this.$emit('fermer')
      } catch (e) {
        // Surface server-side validation errors
        const errs = e.response?.data?.errors || {}
        if (errs.nom)    this.errors.nom    = errs.nom[0]
        if (errs.prenom) this.errors.prenom = errs.prenom[0]
...
.btn-blue:hover:not(:disabled) { background: #2f4f6a; transform: translateY(-1px); }
.btn-blue:disabled { opacity: .65; cursor: not-allowed; }
.btn-outline {
